...
execfile('fiveinarow_play_turn.py')
...
try:
    ...
    while True:
        ...
        if request == 'play_your_turn':
            ...
            if not truthiness_x:
                print 'ERROR: OWNED_BY_X should contain only dicts, but it doesn\'t.'
                continue
            ...
            verify_zero = [type(square) is dict for square in owned_by_zero]
            truthiness_zero = all(verify_zero)
            if not truthiness_zero:
                print 'ERROR: OWNED_BY_X should contain only dicts, but it doesn\'t.'
                continue 
